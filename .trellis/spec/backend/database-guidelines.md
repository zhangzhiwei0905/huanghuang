...
- **A new `CHECK (col IN (...))` enum value is not additive on its own.** `CREATE TABLE IF NOT EXISTS` only runs the CHECK clause for a brand-new database file; an already-deployed database keeps whatever CHECK constraint was baked in when its table was first created, and SQLite has no `ALTER TABLE ... ALTER CHECK`. Adding a new allowed value to a `CHECK (action IN (...))`-style constraint (e.g. adding `RELEASE_WILDCARD` to `competitive_action_events.action`) requires a dedicated rebuild migration: inspect `sqlite_master.sql` for the table, and if its constraint text predates the new value, rename the old table, recreate it with the new CHECK, copy rows across, drop the old table, and recreate any indexes — all inside one transaction with `foreign_keys` temporarily off for the rebuild. See `migrateCompetitiveActionEventsCheckConstraint` in `database.ts` for the reference implementation. Adding a plain column (no CHECK) can keep using the simple `try { ALTER TABLE ... ADD COLUMN } catch { /* duplicate column */ }` pattern; only a CHECK-constrained enum column needs the rebuild.
