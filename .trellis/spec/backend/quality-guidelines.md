...
- Achievement facts are limited to five MATCH actions (`EXPOSED_KONG`, `INDICATOR_PONG_KONG`, `ADDED_KONG`, `CONCEALED_KONG`, `RELEASE_WILDCARD`) and deduplicated by server round/version before counters increment. `PONG` and `WIN` are deliberately excluded — do not add them without a product decision, since `competitiveAchievementAction()` treats "excluded" and "not yet implemented" the same way (returns `null`).
